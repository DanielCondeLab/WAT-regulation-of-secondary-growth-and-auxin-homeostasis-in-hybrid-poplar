...
# This specific script section contains the analysis and output generation for WT vs C1.
# File routes MUST be changed in order to use this code.
# Input files:
# - Bulk raw count matrix for 30 samples
# Output files:
# - One .csv file with all WT vs C1 differential expression results from the additive model
# - One .csv file with WT vs C1 differential expression results filtered at FDR < 0.05
...
# =============================================================================
# CREATE GROUPINGS BY TREATMENT (WT AND C1) AND ZTs (ZT8 AND ZT23)
# =============================================================================
...
keep <- filterByExpr(y) # By default it uses group
...
# The common dispersion is the square root of the BCV. Very, very low values indicate that the experiment may not be reliable
# because there is almost no variability. Very high values indicate high variance between replicates, which reduces the power
# to detect DE genes.

# If “Squeezed” (red) does not change much relative to “Raw” (black), it usually indicates that there were no genes with extreme
# QL dispersions requiring substantial shrinkage.
...
write_csv(zt_final_data, 
          file = '/Users/danielconde/Desktop/WATs/Bulk_edgeR/Results/C1/Aditive/Separated_C1_Aditive_Model.csv')
...
write_csv(zt_final_data, 
          file = '/Users/danielconde/Desktop/WATs/Bulk_edgeR/Results/C1/Aditive/Separated_C1_FDR_005_Aditive_Model.csv')
